...
class IfStatement(Statement):
	...
	#false_block : [Statement] ###CAMBIO1
	false_block : (Statement, type(None)) ##PREGUNTAR2: Como funciona cuando se escribe así
...
class LocalDecl(Statement): ###CAMBIO1
	name : str
	datatype : DataType

class ArrayLocalDecl(Statement): ###CAMBIO1
	name : str
	datatype : DataType

class NullStatement(Statement): ###CAMBIO1
	value : (type(None))

class ReturnStatement(Statement): ###CAMBIO1
	value: (Expression,type(None))

class BreakStatement(Statement): ###CAMBIO1
	value: (type(None))

class CompoundStatement(Statement): ###CAMBIO1
...
class ArrayDeclaration(Statement): ###CAMBIO1
	'''
	var name datatype [ = value ];
	'''
	name     : str
...
class ArrayWriteLocation(Statement): ###CAMBIO1
	location : Location
	value    : Expression
	value1    : Expression
...
class DotVisitor(NodeVisitor):
	...
	###CAMBIO1        # def __init__(self):
	###CAMBIO1        # 	self.dot = pydot.Dot(graph_type='graph', **self._dot_graph_kwargs())
	###CAMBIO1        # 	'''creamos un obj del tipo dot que se va a llamar AST'''
	###CAMBIO1        # 	self.dot.set_node_defaults(**self._dot_node_defaults)
	###CAMBIO1        # 	self.dot.set_edge_defaults(**self._dot_edge_defaults)
	###CAMBIO1        # 	self.st = []
	###CAMBIO1        # 	self.id =0
	def __init__(self):
	...
	def _dot_node_defaults(self, node):
		return {
			'shape': 'box',
			'fontname': 'Curier'
		}
